...
OUTPUT_BIN ?= bin/jobcantan
PREFIX ?= /usr/local
VERSION ?= 0.1.0
# OS ?= $(shell uname -s | tr '[:upper:]' '[:lower:]')
# ARCH ?= $(shell uname -m)
TAR_GZ_FILE_NAME ?= jobcantan-$(VERSION)-darwin-x86_64.tar.gz
TAR_DIR ?= bin/jobcantan
...
tar:
	...
	mkdir -p $(TAR_DIR)
	cp $(OUTPUT_BIN)_bin $(TAR_DIR)/jobcantan
	cp README.md $(TAR_DIR)
	cp LICENSE $(TAR_DIR)
	cd bin && tar zcvf $(TAR_GZ_FILE_NAME) jobcantan
	$(MAKE) sha
